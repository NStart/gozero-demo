...
var (
	...
	cacheBusiness2OsOrgInfoIdPrefix = "cache:business2:osOrgInfo:id:"
)
...
func newOsOrgInfoModel(conn sqlx.SqlConn, c cache.CacheConf, opts ...cache.Option) *defaultOsOrgInfoModel {
...
func (m *defaultOsOrgInfoModel) Delete(ctx context.Context, id int64) error {
	business2OsOrgInfoIdKey := fmt.Sprintf("%s%v", cacheBusiness2OsOrgInfoIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, business2OsOrgInfoIdKey)
	return err
}
...
func (m *defaultOsOrgInfoModel) FindOne(ctx context.Context, id int64) (*OsOrgInfo, error) {
	business2OsOrgInfoIdKey := fmt.Sprintf("%s%v", cacheBusiness2OsOrgInfoIdPrefix, id)
	var resp OsOrgInfo
	err := m.QueryRowCtx(ctx, &resp, business2OsOrgInfoIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultOsOrgInfoModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheBusiness2OsOrgInfoIdPrefix, primary)
}

func (m *defaultOsOrgInfoModel) queryPrimary(ctx context.Context, conn sqlx.SqlConn, v, primary any) error {
	query := fmt.Sprintf("select %s from %s where `id` = ? limit 1", osOrgInfoRows, m.table)
